from core.model.net import Net, Net2, Net3
...
def inference(path_test_video_frames, path_features, ckpt_path, submission_template_path, out_path):
    __C = Cfgs()
    cfg_file = "cfgs/{}_model.yml".format("large")
    ...
    # build answer_id
    dict_ques_ans = dict()
    all_questions = [item for item in open('./all_questions.txt').read().split('\n')]
    all_answers = [item.split(',') for item in open('./all_answers.txt').read().split('\n')]
    ...
    for i_vid, video in enumerate(valid_videos):
        ...
        for s_frame_indices in tqdm(sub_frame_indices):
            ...
            answers = [dataset.ix_to_ans[str(i)] for i in pred_argmax]
...
def parse_args():
    ...
    parser.add_argument('--submission_template_path',
                      ...
                      default='./Test/Annotations/test_questions.json', type=str)
